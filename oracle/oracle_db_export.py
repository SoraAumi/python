...
from oracle_init import db_init
...
def insert_map(data_type, data, column_name, table_name):
    map_str = ""
    if data_type == "CLOB":
        map_str = f"v_{column_name}"
    elif column_name == 'HISTORY_ID':  # 主键 暂时写死 后续优化
...
def get_sql_data(sql, project, env):
    ...
    return list_result, column_property
...
def set_insert_str(result_data, column_property, table_name):
    insert_str_all = ""
    for data in result_data:
        insert_str = ""
        for column in data.keys():
            insert_str += f"{insert_map(column_property[column], data[column], column, table_name)},"
        insert_str_all += f"insert into {table_name} values ({insert_str[:-1]});\n"
    return insert_str_all


def export_db_data(sql, table_name, project, env, file_path="../export/script/"):
    result, column = get_sql_data(sql, project, env)
    print(column)
    # execute_sql = insert_sql.format(declare_sql=set_declare_str(column),
    #                                 insert_sql=set_insert_str(result, column, table_name))
    # create_file_auto(f"{file_path}表{table_name}数据插入.sql", execute_sql)


if __name__ == '__main__':
    export_db_data("select * from HN_PAYMENT_CONDITION_HISTORY where rownum < 2", 'HN_PAYMENT_CONDITION_HISTORY', 'HN',
                   'DEV')
